...
from rag.rag_med import ask_bot2_ai21_segmented, ask_bot_simple_textsplitting
...
print("*** Med 🩺 Documentation Helper***")
...
st.title("Med 🩺 Documentation Helper")
...
if "messages" not in st.session_state.keys():
    st.session_state.messages = [
        {
            "role": "assistant",
            "content": "Ask me a question about Aspirin's drug information?\n"
            "https://www.aspirin.me/sites/g/files/vrxlpx16276/files/2021-02/Aspirin-Protect-en.pdf \n"
...
if st.session_state.messages[-1]["role"] != "assistant":
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            if model == AI21:
                response = ask_bot2_ai21_segmented(question_query=prompt)
            if model == PALM2:
                response = ask_bot_simple_textsplitting(question_query=prompt)
            if model == PALM2_GUARDRAILED:
